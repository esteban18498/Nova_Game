...
void main()
{
    ...
    vec2 pixelPos= aPos + uCenter; // Convert to pixel coordinates (0,0 at center, y-up)
    // Convert to OpenGL normalized coordinates (-1 to 1)
    vec2 normalizedPos;
    normalizedPos.x = (pixelPos.x / uViewportSize.x) * 2.0 ;  // -1 to 1 (left to right)
    normalizedPos.y = (pixelPos.y / uViewportSize.y) * 2.0 ;  // -1 to 1 (bottom to top)
